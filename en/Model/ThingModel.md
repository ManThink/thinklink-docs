...
#### `thingModelId `-Thing Model Identifier 
...
#### `noticeAttrs `-Change notification sign 
...
## Home Assistant integrated configuration **[Note 1] Function enabling requirements**the Home Assistant function must be manually enabled on the device configuration page to be automatically discovered and integrated by the Home Assistant. When this feature is not turned on, the device does not send any discovery messages to Home Assistant.**[Note 2] Configuration Effective Mode **after modifying the field configuration of Home Assistant, you need to go to TKL's ** server Configuration Page**, click **"Re-register all devices"** button, the updated field configuration will take effect and trigger the re-release of device information to Home Assistant [1]. 
...
currently, ThinkLink V2 only natively supports devices **sensor**(Sensor) type access Home Assistant [1]. 
...
### View devices on Home Assistant 
...
2. TKL will use the MQTT topic (the default is `homeassistant/ `prefix) automatically sends a device Discovery message [1]. 
...
### Viewing data through device properties 
